...
// Generated 04-May-2021 13:01:39
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"synthetic_data\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(synthetic_data_M, (NULL));
        synthetic_data_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(synthetic_data_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(synthetic_data_M)
                == (NULL)) || rtmGetStopRequested(synthetic_data_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(synthetic_data_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
